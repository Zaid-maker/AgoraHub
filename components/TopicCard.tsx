...
export default function TopicCard({ id, title, excerpt, author, authorId, category, replies, timeAgo, avatar, voteCount, userVote, moderated }: TopicCardProps) {
    return (
        <article className={`bg-white dark:bg-zinc-900/50 border rounded-xl p-5 card-hover group relative ${moderated ? 'opacity-75 border-amber-200/50 dark:border-amber-900/20 bg-amber-50/10' : ''}`}>
            <div className="flex items-start gap-6">
                ...
                {moderated && (
                    <div className="w-10 h-10 flex items-center justify-center text-amber-500 bg-amber-100/50 dark:bg-amber-900/20 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {moderated ? (
                        <p className="text-xs font-bold text-amber-600/60 uppercase tracking-widest italic mb-4">
                            Content removed for rule violations
                        </p>
                    ) : (
